...
def cells_point_cloud(
    ...
    points = difference_of_gaussians_3d(image, big_sigma/spacing, small_sigma/spacing)
    points = (points >= threshold).astype(np.uint16)
    points = binary_erosion(points, structure=erode_filter).astype(np.uint16)
    if mask is not None:
        points = points * mask
